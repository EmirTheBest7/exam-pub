...
<body>
    <nav class="navbar navbar-light">
        <div class="container">
            ...
            <?php
            include("../../db.php");
            $sel_drink="Select * from drinks;";
            $drink_query = mysqli_query($con,$sel_drink);
            while($drink = mysqli_fetch_assoc($drink_query)) {
                echo '<a href=?drink_id='.$drink['drink_id'].'>'.$drink['product'].'_' .$drink['amount'].'</a>';
            ...
            <table width="100%" border="1" style="border-collapse:collapse;">
                ...
                <tbody>
                    <?php
                        ...
                        $records = mysqli_query($con, "SELECT * FROM orders WHERE what=".$drink_id."");
                        while($data = mysqli_fetch_assoc($records)) { ?>
                    <tr>
                        <td align="center"><?php echo date('h:i:sa', strtotime($data['time'])); ?></td>
                        <?php
                        $drink_id = $data["what"];
